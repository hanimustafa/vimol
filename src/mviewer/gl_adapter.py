...
``Style`` (mviewer) and ``SphereBatch``/``CylinderBatch``/``ShadingParams``
(``gl_render.py``, which otherwise has zero mviewer imports).
"""
...
from .gl_render import SphereBatch, CylinderBatch, ShadingParams
...
def molecule_to_gl_inputs(molecule: Molecule, camera: Camera, style: Style,
                          ...
                          ) -> Tuple[SphereBatch, CylinderBatch, np.ndarray, ShadingParams]:
    ...
    if draw_bonds:
        ...
        cylinders = CylinderBatch(
            a=vpos[idx_i],
            b=vpos[idx_j],
            radii=np.full(len(molecule.bonds), rb, dtype=np.float32),
            colors_a=colors[idx_i],
            colors_b=colors[idx_j],
        )
    else:
        cylinders = CylinderBatch.empty()
    ...
    return spheres, cylinders, proj, shading
